...
func (w *Watcher) generateOrderEventsIfChanged(
	...
	for _, acceptedOrderInfo := range validationResults.Accepted {
		order := w.findOrder(acceptedOrderInfo.OrderHash)
		if order == nil || order.LastRevalidatedBlockNumber.Int64() >= validationBlockNumber.Int64() {
			// Noop if order no longer in DB OR it's already been re-validated at this block height or higher
			continue
		}
...
func (w *Watcher) ValidateAndStoreValidOrders(orders []*zeroex.SignedOrder, pinned bool, chainID int) (*ordervalidator.ValidationResults, error) {
	...
	for i, acceptedOrderInfo := range results.Accepted {
		...
		orderEvents = append(orderEvents, orderEvents...)
	}
